...
def fetch_live_data():
    ...
    c.execute('SELECT timestamp, temp, switch FROM sensor_data WHERE time >= ? AND time <= ?', (start_datetime, end_datetime))
    data = c.fetchall()
...
def fetch_custom_data():
    ...
    conn = sqlite3.connect('/path/to/your/sensor_data.db')
    c = conn.cursor()

    # Fetch data from the database based on the user-defined date and time range
    c.execute('SELECT timestamp, temp, switch FROM sensor_data WHERE timestamp BETWEEN ? AND ?', (start_datetime, end_datetime))
    data = c.fetchall()
